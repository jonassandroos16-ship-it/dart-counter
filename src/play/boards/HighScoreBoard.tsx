...
import { useHighScoreBoard } from './HighScoreBoardLogic';
import { useToast } from '../toast';

export function HighScoreBoard({ game, setGame, players, games, settings }: {
  game: Game; setGame: (g: Game) => void; players: Player[]; games: GameRecord[]; settings: Settings;
}) {
  const toast = useToast();
  const { enterVisit, addDart } = useHighScoreBoard(game, setGame, settings, toast);
  ...
  return (
    <div className="board highscore-board">
      <div className="board-header" style={{ background: themeColor }}>
        <span className="board-title">{MODES.highscore.name}</span>
