...
export const CARD_BADGES: BadgeDef[] = [
  ...
  { id: 'cb_spell_master', name: 'Spell Master', icon: '✨', desc: 'Play 50 spell cards across all games.', kind: 'post-game', contextLabel: 'Spells cast', context: (_pid, games) => games.reduce((a: number, g: any) => a + g.players.reduce((pa: number, pl: any) => pa + (pl.visits || []).reduce((va: number, v: any) => va + (v.darts || []).filter((d: any) => d.label?.includes('Spell')).length, 0), 0), 0) },
  { id: 'cb_card_collector', name: 'Card Collector', icon: '📦', desc: 'Collect 15 different cards.', kind: 'post-game', contextLabel: 'Cards owned', context: () => 0 },
];
