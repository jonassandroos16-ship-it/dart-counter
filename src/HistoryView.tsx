            ...
            <div className="row between">
              <div>
                <div className="row" style={{ gap: 8 }}>
                  ...
                  <span className="muted small">{new Date(g.date).toLocaleDateString()} {new Date(g.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
  ...
  return (
    <Modal onClose={onClose}>
      <div style={{ maxHeight: '70vh', overflow: 'auto' }}>
        ...
        <div className="muted small" style={{ marginBottom: 12 }}>{new Date(g.date).toLocaleString()}</div>
        {g.players.map(p => {
          if (g.atc) {
